...
**Task:** S1-T02 (layer contract). **As implemented through S2-T02:** App Router at `frontend/app/` (no `src/`); layers in S1-T04; static catalog in S1-T05; Vitest in S1-T06; Option 1 tokens + semantic shell in S1-T07; category listing at `/c/[slug]` (S2-T01; S2-T02 closed without a second route).
...
**S2-T01 / S2-T02:** `getCategoryPage(categories, products, slug)` returns `{ category, products }` or `null` (unknown slug). S2-T02 did not add a second listing use case. Pages call `config/catalog.ts`, not fixtures. View models map domain → `ProductCard` props (no price/inventory).
...
## 17. Folder structure (as implemented through S2-T02)
...
There is **no S1-T09**. S2-T01 and S2-T02 are complete: `/c/[slug]` (built in S2-T01; S2-T02 added no duplicate route). Next: **S2-T03** — do not start automatically. Do not implement PDP in this file’s prior tasks.
